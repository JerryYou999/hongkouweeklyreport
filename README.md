...
开发服务器会打印本地访问地址。需要先按 `cloudbase/README.md` 配置本地 CloudBase HTTP API 地址。
...
- CloudBase PostgreSQL：周报元数据、版本、搜索正文、章节标题和限速计数；
- CloudBase PostgreSQL 原生云存储：原始 HTML/PDF；
...
本项目按需求不显示登录或上传权限，因此任何知道网站地址的人都可以查看、搜索和上传。网页通过 CloudBase HTTP API 请求数据；数据库和原文件不直接暴露给浏览器。HTML 原文件只在隔离的 iframe 中预览，不会在主页面上下文中执行脚本。
