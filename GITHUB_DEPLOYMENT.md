...
- Cloudflare R2：保存 HTML/PDF 原文件和清理后的 HTML。
...
2. 在 Cloudflare 创建名为 `hongkou-weekly-report` 的 D1 数据库，以及名为 `hongkou-weekly-reports` 的 R2 存储桶。
...
5. 在 GitHub 仓库的 Actions secrets 添加：
   - `CLOUDFLARE_API_TOKEN`：仅授予该 Worker、D1、R2 的编辑权限。
...
当前 Sites 中的 D1/R2 数据不会自动出现在新 Cloudflare 账号。正式切换前需要执行一次导出与导入；在确认新站数据、预览、下载和搜索无误前，不应关闭旧站。若目前全部是测试数据，也可以在新站重新上传原文件，系统会重新生成索引和版本记录。
...
| Cloudflare R2 | 通常 ¥0 | 原文件超过免费存储/请求额度后计费；R2 公网出口流量目前免费 |
...
对于目前“一周一份、内部转发访问”的规模，D1 和 R2 大概率长期处于免费额度。最可能的固定成本只有自定义域名；如果 PDF 在 Worker 免费 CPU 限额内解析不稳定，再增加约 US$5/月的 Workers 付费计划。
